...
if __name__ == "__main__":
    ...
    for name, group in grouped_dataframe:
        ...
        total_runtime = group["runtime"].sum() + empty_runtime_rows * timeout
        par_X = (total_runtime + ((X_in_parX - 1) * timeouts * timeout)) / num_rows
        table_data.append([name, num_rows, timeouts, round(total_runtime, 2), par_X])
    table_df = pd.DataFrame(table_data, columns=["Algorithm", "N", "#TO", "RT", "PAR"+ str(X_in_parX)])
